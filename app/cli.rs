...
use clap::{Arg, Parser};
use plain_bitassets::{
    parent_chain::config::{ParentChainConfig, ParentChainType},
    types::{Network, THIS_SIDECHAIN},
};
use std::str::FromStr;
use url::{Host, Url};
...
pub(super) struct Cli {
    ...
    /// Parent chain node URL (format: CHAIN:URL, e.g., BTC:http://localhost:8332)
    /// Can be specified multiple times for different chains
    /// Supported chains: BTC, BCH, LTC, XMR, ETH, TRON
    #[arg(long, value_name = "CHAIN:URL")]
    pub parent_chain_node: Vec<String>,
    /// Path to parent chain configuration file (JSON format)
    /// If specified, overrides individual --parent-chain-node arguments
    #[arg(long)]
    pub parent_chain_config: Option<PathBuf>,
}
...
impl Cli {
    ...
    pub fn get_config(self) -> anyhow::Result<Config> {
        ...
        // Parse parent chain configuration
        let parent_chain_config = if let Some(config_path) = &self.parent_chain_config {
            // Load from file
            let config_str = std::fs::read_to_string(config_path)?;
            serde_json::from_str(&config_str)?
        } else {
            // Parse from command line arguments
            let mut config = ParentChainConfig::new();
            for chain_node in &self.parent_chain_node {
                let parts: Vec<&str> = chain_node.splitn(2, ':').collect();
                if parts.len() != 2 {
                    return Err(anyhow::anyhow!(
                        "Invalid parent chain node format: {}. Expected CHAIN:URL",
                        chain_node
                    ));
                }
                let chain = ParentChainType::from_str(parts[0])
                    .map_err(|_| anyhow::anyhow!("Unknown chain: {}", parts[0]))?;
                let url = Url::parse(parts[1])
                    .map_err(|e| anyhow::anyhow!("Invalid URL {}: {}", parts[1], e))?;
                config.set_chain(chain, url, None, None);
            }
            config
        };
...
pub struct Config {
    ...
    pub rpc_host: Host,
    pub rpc_port: u16,
    /// Parent chain configuration for CoinShift swaps
    pub parent_chain_config: ParentChainConfig,
    #[cfg(feature = "zmq")]
    pub zmq_addr: SocketAddr,
}
